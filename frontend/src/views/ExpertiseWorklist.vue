  ...
  <div class="page-enter">
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Daftar pasien yang sudah selesai dirotgen dan menunggu diisi ekspertise/bacaan.</p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
              ...
              <th class="px-6 py-3.5 text-left">Tgl. Selesai</th>
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
            <tr v-for="h in paginated" :key="h.id" class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-slate-500 font-medium">
                {{ new Date(h.orderDate).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' }) }}
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-900/30 text-amber-600 border border-amber-200 dark:border-amber-800">
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Butuh Bacaan
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  <router-link :to="`/admin/orders/${h.id}?from=expertise`" class="text-[11px] font-bold px-4 py-2 rounded-lg bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-light)] transition-colors shadow-sm flex items-center gap-1.5">
                  ...
                  <button @click="updateStatus(h.id, 'scheduled')" class="text-[10px] font-bold px-2 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 border border-blue-100 dark:border-blue-800 hover:bg-blue-100 transition-all flex items-center gap-1" title="Kembalikan ke Radiografer">
      ...
      <div v-else class="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p class="font-bold text-slate-600 dark:text-slate-300">Semua Order Telah Dibaca</p>
        <p class="text-slate-400 text-sm mt-1">Tidak ada tugas bacaan yang menunggu.</p>
      </div>
    </div>
